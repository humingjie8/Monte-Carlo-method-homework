...
for i in tqdm(range(cycle), desc="Processing"):
    ...
    for x in range(number_data_t):
        ...
        standard_normal_samples = np.random.standard_normal(size=number_data_z)  # 生成10000个样本
        mean_value = np.mean(standard_normal_samples)
        min_value = np.min(standard_normal_samples)

        # for i in range(number_data):
        mid_1 = sum_of_first_n_elements((standard_normal_samples-mean_value)**2, number_data_z)
    ...
    # 在EXCEL中写入数据
    sheet.cell(row=i+2, column=1, value=str(number_data_z))  # 在第一列写入数据
    ...
    # #SAVE PIC
    # name_pic = 'stand_pic/'+'stand'+str(number_data_z)+'.png'
...
# 保存Excel文件
name_excel = 'stand'+'.xlsx'
workbook.save(name_excel)

#SAVE PIC
# name_pic = 'stand_pic/'+'stand'+'.png'
# plt.savefig(name_pic)
